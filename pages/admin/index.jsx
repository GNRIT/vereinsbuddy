                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {stats.letzteEinsaetze?.map((einsatz) => (
                                        <tr key={einsatz.ID}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{einsatz._count?.teilnahmen || 0}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
...
export async function getServerSideProps(context) {
    ...
    const durchschnittTeilnahme = teilnahmeStats.length > 0 
        ? Math.round(teilnahmeStats.reduce((sum, stat) => sum + stat._count.ID, 0) / teilnahmeStats.length)
        : 0
        ...
        props: {
            stats: {
                ...
                aktivsteMitglieder: JSON.parse(JSON.stringify(aktivsteMitglieder.map(m => ({
                    ...m,
                    letzteTeilnahme: m.teilnahmen?.[0]?.Erstellt_am || null
                }))))
            }
        }
    }
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Einsatz</th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {stats.letzteEinsaetze?.map((einsatz) => (
                                        <tr key={einsatz.ID}>
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{einsatz.Titel}</td>
                                            <td className="px-6 py-4 text-sm text-gray-500">{einsatz._count?.teilnahmen || 0}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Teilnahmen</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Letzte Teilnahme</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {stats.aktivsteMitglieder?.map((mitglied) => (
...
export async function getServerSideProps(context) {
    ...
    const aktiveMitgliederFF = await db2.mitglied.count({
        where: { Status: 'Aktiv', Abteilung: 'FF' }
    })

    const aktiveMitgliederJF = await db2.mitglied.count({
        where: { Status: 'Aktiv', Abteilung: 'JF' }
    })
    ...
    let aktivsteMitglieder = await db2.mitglied.findMany({
        where: { Status: 'Aktiv' },
        include: {
            _count: { select: { teilnahmen: true } },
            teilnahmen: {
                orderBy: {
                    Erstellt_am: 'desc'
                },
    ...
    aktivsteMitglieder = aktivsteMitglieder
        .sort((a, b) => (b._count.teilnahmen ?? 0) - (a._count.teilnahmen ?? 0))
        .slice(0, 5)
    ...
    const durchschnittTeilnahme = teilnahmeStats.length > 0
        ? Math.round(
            teilnahmeStats.reduce((sum, stat) => sum + stat._count.ID, 0) / teilnahmeStats.length
        )
        : 0
